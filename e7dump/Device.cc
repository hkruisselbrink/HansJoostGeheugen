...
#include "unix_error.h"		// throwing unix errors
	...
	, requests(0)
{
	...
	// Open the image file
	#ifdef O_BINARY
     fd = open(file, O_RDONLY|O_BINARY); // tell windows to keep it's hands off our data!
    #else
     fd = open(file, O_RDONLY);
    #endif
	if (fd < 0) {						// some problem?
		throw unix_error(file);			// report the problem
	}

	// Inspect the file attributes
	struct stat  buf;
	if (fstat(fd, &buf) < 0) {			// request the file description
		throw unix_error("fstat");		// oops!
	}
...
Block	*Device::getBlock(daddr_x  blocknumber)
{
	...
	if (lseek(fd, blockpos, SEEK_SET) != blockpos) {
		throw unix_error("seeking block");
	}

	// Get that block in core
	Block  *bp = new Block();						// create a new, empty, block
	size_t  n = read(fd, bp->rawdata, DBLKSIZ);		// and fill it's u union with raw data
	if (n != DBLKSIZ) {
		throw unix_error("reading block");
	}
